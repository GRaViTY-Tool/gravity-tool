...
public class MOMotExecutor {
	...
	protected final String[] unitsToRemove = new String[] { "MoveMethod::rules::libCheck",
			"MoveMethod::rules::MoveMethod", "MoveMethod::rules::checkPreconditions",
			"MoveMethod::rules::dynMoveMethod", "MoveMethod::rules::changeVisibility" };
	...
	// ------Fitness----------------
	protected IFitnessDimension<TransformationSolution> getCouplingFitness(
	...
	protected IFitnessDimension<TransformationSolution> getTransformationLengthDimension(
			final TransformationSearchOrchestration orchestration) {
		IFitnessDimension<TransformationSolution> dimension = new TransformationLengthDimension();
		dimension.setName("SolutionLength");
		dimension
				.setFunctionType(at.ac.tuwien.big.moea.search.fitness.dimension.IFitnessDimension.FunctionType.Minimum);
		return dimension;
	}
	...
	protected IFitnessDimension<TransformationSolution> _createConstraint_0(
			final TransformationSearchOrchestration orchestration) {
		return new AbstractEGraphFitnessDimension("visibility",
				at.ac.tuwien.big.moea.search.fitness.dimension.IFitnessDimension.FunctionType.Minimum) {
			@Override
			protected double internalEvaluate(TransformationSolution solution) {
				EGraph graph = solution.execute();
	...
	protected TransformationResultManager handleResults(final SearchExperiment<TransformationSolution> experiment,
			File output) {
		File goblinFolder = new File(output, "goblin");
		...
		File objectivesFile = new File(goblinFolder, "objectives/objective_values.txt");
		System.out.println("- Save objectives of all algorithms to '" + objectivesFile.toString() + "'");
		TransformationResultManager.saveObjectives(objectivesFile.getAbsolutePath(), population);
		...
		File objectiveValuesFile = new File(goblinFolder, "solutions/objective_values.txt");
		System.out.println("- Save solutions of all algorithms to '"
				+ objectivesFile.toString()
				+ "'");
		TransformationResultManager.savePopulation(objectiveValuesFile.getAbsolutePath(), population, populationWriter);

		File solutionsFolder = new File(goblinFolder, "solutions");
		System.out.println("- Save solutions of all algorithms to '" + solutionsFolder.toString() + "'");
		...
		File modelsFolder = new File(goblinFolder, "models");
		System.out.println("- Save models of all algorithms to '" + modelsFolder.toString() + "'");
		TransformationResultManager.saveModels(modelsFolder.getAbsolutePath(), baseName, population);
	...
	public TransformationResultManager performSearch(final String initialGraph, final int solutionLength, File output) {
		...
		System.out.println("-------------------------------------------------------");
		return handleResults(experiment, output);
	}
	...
	public static void main(final String... args) {
		...
		search.performSearch(INITIAL_MODEL, SOLUTION_LENGTH, new File("output"));
		finalization();
	}
}
