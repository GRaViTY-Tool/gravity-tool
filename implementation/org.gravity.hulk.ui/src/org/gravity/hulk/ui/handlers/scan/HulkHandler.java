...
public abstract class HulkHandler extends AbstractHandler {
	...
	public void initHulk() {
		...
		final var pmResource = pg.eResource();
		final var rs = pmResource.getResourceSet();
		final var antiPatternResource = rs.createResource(URI.createURI("platform:/resource/" + pg.getTName() + '/' //$NON-NLS-1$
		...
		final var dependencies = this.hulk.getDependencyGraph();

		final var res = rs.createResource(URI.createURI("SmellDependencyGraph.xmi")); //$NON-NLS-1$
	...
	public IPath copyAnnotationsJar() throws IOException {
		final var annotations = this.getAnnotationsOut(this.hHmonitor);
	...
	public void initializeSelection(final AntiPatternSelectionDialog dialog) {
		...
		for (final Object selected : dialog.getResult()) {
			if (selected instanceof final Class<?> eClass) {
				if (HDetector.class.isAssignableFrom(eClass)) {
					this.selection.add((Class<HDetector>) eClass);
				}
			}
		}
	}
	...
	public void displayResults() {
		final Runnable runnable = () -> (new ResultDialog(HulkHandler.this.window.getShell(),
	...
	public void runJob() {
		...
		this.job = new Job(this.jobName) { // $NON-NLS-1$
			...
			protected IStatus run(final IProgressMonitor monitor) {
				...
				final var projects = SelectionHelper.getJavaProjects(HulkHandler.this.workspaceSelection);
				...
				for (final IJavaProject javaProject : projects) {
					...
					if (HulkHandler.this.javaAnnotationsEnabled) {
						IPath annotations;
						try {
							annotations = HulkHandler.this.copyAnnotationsJar();
					...
					try {
						HulkHandler.this.converter = GravityActivator.getDefault()
								.getNewConverter(HulkHandler.this.project.getProject());
					...
					final var success = HulkHandler.this.converter.convertProject(libs, monitor);
					if (!success || (HulkHandler.this.converter.getPG() == null)) {
						LOGGER.log(Level.ERROR,
								"Creating PG from project failed: " + HulkHandler.this.project.getProject().getName());
						fails.add(javaProject.getProject().getName());
						continue;
					}

					HulkHandler.this.initHulk();

					final var t3 = System.currentTimeMillis();
					LOGGER.log(Level.INFO, t3 + " Sync Bwd");

					HulkHandler.this.syncBWD();

					IFolder hulkFolder;
					try {
						hulkFolder = EclipseProjectUtil.getGravityFolder(HulkHandler.this.project.getProject(),
								monitor);
					} catch (final IOException e) {
					...
					final var pmXmi = hulkFolder.getFile(HulkHandler.this.project.getProject().getName() + ".xmi");
					...
					HulkHandler.this.displayResults();
